...
export default function Portfolio (props: {portfolioItem: porfoliosResponseT, refetch: ()=>void}){

  const [portfolioItemState, setPortfolioItemState] = React.useState(props.portfolioItem);
        ...
        <div aria-label='portfolio container' className={`max-w-min`}>
          <div
            ...
            className={`border-2 border-primary rounded-md
                      ...
                      `}
          >
            <button onClick={editPortfolio} className={`absolute top-2 right-2 text-xl`}>
              <FaPen />
            </button>
            <span aria-label='number of projects'>
              {portfolioItemState.doneProjectsNumber}/{portfolioItemState.projectsNumber}
            </span>
          </div>
